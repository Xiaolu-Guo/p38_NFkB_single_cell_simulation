...
% clear all
function [] = para_sample_fitting_sim_sti_doses_var_2023(data_save_file_path,input_paras,cal_codon,save_metric_name,fitting_dose)
...
for i_proj_num_vec = 1:length(proj_num_vec)
    ...
    switch length(proj_num_vec{i_proj_num_vec})
        case 1
            ...
            fold_change_log_MKK4 = log(10^0) + rand_mat_MKK4 * (log(2)/3);
            fold_change_MKK4 = exp(fold_change_log_MKK4);
            
            rand_mat_MKK6 = randn(1,Num_sample(i_proj_num_vec));
            fold_change_log_MKK6 = log(10^0) + rand_mat_MKK6 * (log(2)/3);
            fold_change_MKK6 = exp(fold_change_log_MKK6);
            
            rand_mat_p38 = randn(1,Num_sample(i_proj_num_vec));
            fold_change_log_p38 = log(10^0) + rand_mat_p38 * (log(2)/3);
            fold_change_p38 = exp(fold_change_log_p38);
