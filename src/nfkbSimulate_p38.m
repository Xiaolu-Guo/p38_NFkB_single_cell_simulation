function [t,x, simdata, x_end] = nfkbSimulate_p38(stimulus, names, params_mod, init_mod, options)
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
if isfield(v,'init_val') && isfield(v.init_val,'MKK6_off')
    v.SPECIES.INIT(strcmp('MKK6_off',v.SPECIES.NAMES))   = v.init_val.MKK6_off;
else
    v.SPECIES.INIT(strcmp('MKK6_off',v.SPECIES.NAMES))   = 0.1;
end

if isfield(v,'init_val') && isfield(v.init_val,'MKK4_off')
    v.SPECIES.INIT(strcmp('MKK4_off',v.SPECIES.NAMES))   = v.init_val.MKK4_off;
else
    v.SPECIES.INIT(strcmp('MKK4_off',v.SPECIES.NAMES))   = 0.1;
end

if isfield(v,'init_val') && isfield(v.init_val,'p38_off')
    v.SPECIES.INIT(strcmp('p38_off',v.SPECIES.NAMES))   = v.init_val.p38_off;
else
    v.SPECIES.INIT(strcmp('p38_off',v.SPECIES.NAMES))   = 0.1;
end
...
if ~isfield(v,'STEADY_STATE')
    v.PHASE     = 1;
    [~, r1] = ode15s('nfkbOde_p38', [v.T_EQUILIBRATE 0], v.SPECIES.INIT,ode_opt,v);
    ...
    converge = 0;
    iter = 0;
    %    nfkbOde_p38([],[],[],v); % Create global delay variable
    while ~converge
        [~, r1] = ode15s('nfkbOde_p38', [v.T_EQUILIBRATE 0], starting_vals,ode_opt,v);
...
v.PHASE = 2;
[t_sim, x_sim] = ode15s('nfkbOde_p38', v.SIM_TIME,starting_vals,ode_opt,v);
